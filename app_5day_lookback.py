"""
Cotton Options Pricing — 5-Day AWP Lookback
...
def calculate_modified_strike(stock_prices, ref_price, carry_rate,
    ...
    final_strike = ref_price + carry_rate * days_elapsed   # Loan + Carry only, no LRS in ceiling
    if days_elapsed == 0:
        return final_strike
    awp = calculate_awp_for_day(stock_prices, days_elapsed, lrs)
...
def _collect_payoffs(S, exercise_indicators, ref_price, carry_rate, lrs,
    ...
    """
    CALL: sell at full S, repay at K = min(AWP, Loan+Carry). payoff = S - K.
          Lookback tracks minimum K (cheapest repayment) over 5-day post-exercise window.
    PUT:  payoff = max(K - AWP, 0). Lookback tracks minimum AWP.
    """
    dt                = 1 / 365.0
    ...
    for i in range(n_simulations):
        ...
        # 5-day lookback: call tracks min K; put tracks min AWP
        best = k_ex
        for offset in range(1, POST_EX_DAYS + 1):
            ...
            if mon < S.shape[1]:
                if direction == 'call':
                    ...
                    best = min(best,
                               calculate_awp_for_day(S[i, :mon + 1], mon, lrs))
        lb = max(k_ex - best, 0)
...
def price_american_put(S0, loan_value, lrs, r, sigma, days_to_maturity, n_simulations):
    ...
    for t in range(days_to_maturity + 1):
        for i in range(n_simulations):
            k        = calculate_modified_strike(S[i, :t + 1], loan_value, CARRY_RATE, t, lrs)
            ev[i, t] = max(k - (S[i, t] - lrs), 0)   # put: K > AWP = S - LRS
    _, ei = _run_lsm(S, ev, r, days_to_maturity, n_simulations)
    k0  = calculate_modified_strike([S0], loan_value, CARRY_RATE, 0, lrs)   # = loan_value
    ...
    res.update({
        ...
        'intrinsic':           max(k0 - (S0 - lrs), 0),   # K - AWP at inception
...
def chart_fan(S0, r, sigma, T, loan, lrs):
    ...
    floor         = [loan + CARRY_RATE * d for d in days]
...
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown('## 🌾 Cotton Options Pricer <span class="lb-badge">5-DAY LOOKBACK</span>',
            unsafe_allow_html=True)
st.markdown("American-style options with **5-day AWP lookback** — Longstaff-Schwartz Monte Carlo (LSM)")
...
if st.session_state.call_res is not None:
    ...
    st.caption("Profits when cotton rises above the modified strike. Includes 5-day AWP lookback.")
    st.markdown("")
    render_result_card("② American Put — Cost of Downside Protection", "put", pr)
    st.caption("Cost of downside insurance. Includes 5-day AWP lookback.")
    ...
    if sr is not None:
        ...
        st.caption("Fair value of downside exposure from short entry. 5-day AWP lookback.")
...
st.caption(
    ...
    "5-day post-exercise AWP lookback \u00b7 "
